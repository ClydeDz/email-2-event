...
A Chrome extension that adds actions to an open Gmail message — **Create event in Google Calendar** and optionally **Create task in Google Tasks** — uses Chrome's built-in on-device LLM (Gemini Nano) to extract the key fields from the email, and pre-populates Google's native create dialog so the user just clicks Save.

**Note:** The **Create task** feature is behind a feature flag (`VITE_ENABLE_TASK_CREATION`). By default, only event creation is enabled. Set `VITE_ENABLE_TASK_CREATION=true` to enable task creation functionality, which requires OAuth and Google Tasks API access.
...
| Pre-fill Google Tasks before saving                      | ⚠️               | No public template URL exists for Tasks. v1 uses an in-page **review modal** with editable pre-filled fields; on Save it calls Tasks API v1 `tasks.insert` (OAuth required). **Behind feature flag `VITE_ENABLE_TASK_CREATION`.** | Low — same UX (user clicks Save), but implementation is API-backed rather than URL-backed. Only available when feature flag is enabled.                                                                                                                                   |
| Google OAuth (for the Tasks path + profile)              | ✅               | `chrome.identity.getAuthToken` with `openid profile email tasks`. **Only when `VITE_ENABLE_TASK_CREATION` is enabled.**                                                                                                           | Low. Calendar path needs no OAuth in v1 because we use the template URL. OAuth is only required for task creation (behind feature flag).                                                                                                                                  |
| Public distribution / any Google account                 | ✅               | Chrome Web Store listing; Google OAuth consent screen set to **External / In production** (only when `VITE_ENABLE_TASK_CREATION` is enabled)                                                                                      | When task creation is enabled, the `tasks` scope is non-sensitive, so OAuth verification is lightweight (mainly brand review). `profile`/`email` are basic scopes. Calendar template URL avoids any Calendar scope. When task creation is disabled, no OAuth is required. |
...
4. **Tasks feature flag.** Task creation (including OAuth and Tasks API integration) is behind the `VITE_ENABLE_TASK_CREATION` feature flag. By default, only event creation is available. This allows for a simpler initial release without requiring Google OAuth verification for the non-sensitive `tasks` scope.

5. **Tasks vs Events for "invoice".** A Google Task has a `due` _date_ (no time) and `notes` — perfect for "pay £450 by 30 May". A Calendar event needs start/end times. The two buttons let the user pick the target; the LLM's job is just field extraction, not classification. (When task creation is enabled via feature flag.)

6. **Privacy.** Email content stays on-device in v1 (Gemini Nano is local). The privacy policy will say so explicitly — a strong differentiator for a public extension.
